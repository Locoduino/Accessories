...
int ShiftRegister74HC595::IdCounter = EXPPREFIX74HC595;
...
ShiftRegister74HC595::ShiftRegister74HC595(int numberOfShiftRegisters, int serialDataPin, int clockPin, int latchPin)
{
    ...
    _latchPin = latchPin;
		...
		CString name;
		name.Format(_T("74HC595-%X"), IdCounter - EXPPREFIX74HC595 + 1);
		this->Id = IdCounter++;
		AddExpander(this->Id, 8 * numberOfShiftRegisters, OUTPUT, name);
...
void ShiftRegister74HC595::set(int pin, uint8_t value)
{
	...
	pinStateRaw(pin, this->Id, value == 1 ? HIGH : LOW);
}
...
/*    for (int i = _numberOfShiftRegisters - 1; i >= 0; i--) {
	...
	for (int iPin = 0; iPin < 8 * _numberOfShiftRegisters; iPin++)
	{
		pinStateRaw(iPin, this->Id, get(iPin) == 1 ? HIGH : LOW);
	}
