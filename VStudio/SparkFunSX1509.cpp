...
int SX1509::IdCounter = SX1509EXPPREFIX;
...
SX1509::SX1509(byte address, byte resetPin, byte interruptPin, byte oscillatorPin)
{
	...
	pinMode(interruptPin, OUTPUT_INTERRUPT);
...
{
	CString name;
	name.Format(_T("SX1509-%X"), address);
	this->Id = IdCounter++;
	AddExpander(this->Id, 16, INPUT, name);
	for (int i = 0; i < 16; i++)
	{
		types[i] = INPUT;
	}
...
{
	// unknown mode ANALOG_OUTPUT !
	::pinMode(pin, this->Id, inOut);
	types[pin] = inOut;
}
...
{
	if (types[pin] == OUTPUT)	// If the pin is an output, write high/low
	{
		::digitalWrite(pin, this->Id, highLow);
	}
...
{
	return ::digitalRead(pin, this->Id);
}
...
void SX1509::pwm(byte pin, byte iOn)
{
	...
	// Linear mode: Ion = iOn
	// Log mode: Ion = f(iOn)
	::analogWrite(pin, this->Id, (int)iOn);
...
{
	return 0;// readKeyData();
}

unsigned int SX1509::readKeyData()
{
	return 0;// (0xFFFF ^ readWord(REG_KEY_DATA_1));
}
...
{
	::pinMode(pin, this->Id, OUTPUT_INTERRUPT);
}
